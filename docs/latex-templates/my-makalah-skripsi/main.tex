...
Deployment costs are one-time expenses that cooperatives bear during initial system setup. Per-operation costs range from 72K to 210K gas, comparable to existing DChain-deployed applications such as OrvynLabs (78K--89K gas per vote and donation), confirming that the Lakomi contracts operate within the same cost envelope as prior DChain implementations. For a cooperative with 100 active members performing monthly simpanan payments, annual voting, and quarterly loans, the estimated total annual operational gas cost is approximately 42M gas, well within practical bounds for a consortium blockchain environment.
