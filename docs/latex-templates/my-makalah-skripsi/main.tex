...
\and
\IEEEauthorblockN{Ir. Noor Akhmad Setiawan, S.T., M.T., Ph.D., IPM.}
\IEEEauthorblockA{\textit{Department of Electrical and Information Engineering} \\
\textit{Universitas Gadjah Mada}\\
Yogyakarta, Indonesia \\
noorwewe@ugm.ac.id}
...
This paper presents \textbf{Lakomi}, a blockchain-based cooperative system that directly addresses this gap by encoding 26 provisions of UU 25/1992 and its derivative regulations into four interoperable smart contracts---LakomiToken (membership), LakomiVault (treasury and SHU), LakomiGovern (democratic governance and elections), and LakomiLoans (collateralized lending)---deployed on DChain (EVM-compatible, chain ID 17845), a consortium blockchain for Indonesian higher education. The primary contributions of this work are:
...
\begin{enumerate}
    ...
    \item A dual-track architecture that separates governance power---strictly one-member-one-vote per Pasal 22(1)---from financial privileges---contribution-based loan-to-value tiers per Pasal 18---resolving the DAO plutocracy problem while maintaining cooperative legal compliance.
...
\centering
\begin{tikzpicture}[
    box/.style={rectangle, rounded corners=3pt, draw, minimum width=2.6cm, minimum height=0.55cm, font=\footnotesize, align=center, inner sep=4pt},
    smallbox/.style={rectangle, rounded corners=2pt, draw, fill=gray!5, minimum width=2.6cm, minimum height=0.4cm, font=\tiny, align=left, inner sep=3pt},
    rolebox/.style={rectangle, rounded corners=1pt, draw=green!40, fill=green!6, minimum width=2.4cm, minimum height=0.28cm, font=\tiny\itshape, align=center, inner sep=2pt},
    arr/.style={-{Stealth[length=3pt]}, thick},
    darr/.style={-{Stealth[length=3pt]}, thick, dashed, gray!50},
]

% === MEMBERSHIP LAYER ===
\node[box, fill=purple!10, minimum width=8.5cm] (token)
    at (0,4.0) {LakomiToken (LAK)\\ERC-20 Membership Token $\mid$ \texttt{isRegisteredMember}};
\node[rolebox, below=0.08cm of token]
    {\textsc{Minter} $\cdot$ \textsc{Burner} $\cdot$ \textsc{Locker} $\cdot$ \textsc{Membership}};
\begin{pgfonlayer}{background}
    \node[draw=purple!40, fill=purple!2, rounded corners=5pt,
          fit=(token), inner sep=8pt,
          label={[font=\tiny\bfseries]above:Membership Layer}] {};
\end{pgfonlayer}

% === ARROWS DOWN ===
\draw[arr] (-2.8,3.4) -- (-2.8,1.65);
\draw[arr] (0,3.4)    -- (0,1.65);
\draw[arr] (2.8,3.4)  -- (2.8,1.65);

% === CONTRACT LAYER ===
\node[box, fill=blue!8] (vault)  at (-2.8,1.2)  {LakomiVault\\USDC Treasury \& Simpanan};
\node[box, fill=blue!8] (govern) at (0,1.2)     {LakomiGovern\\1-Member-1-Vote\\Proposals \& Elections};
\node[box, fill=blue!8] (loans)  at (2.8,1.2)   {LakomiLoans\\Collateralized Loans\\5\% APY $\cdot$ Tiered LTV};

\node[rolebox, below=0.06cm of vault]  (vrole)  {\textsc{Treasurer}};
\node[rolebox, below=0.06cm of govern] (grole)  {\textsc{Pengawas}};
\node[rolebox, below=0.06cm of loans]  (lrole)  {\textsc{Approver}};

\node[smallbox, below=0.12cm of vrole, text width=2.8cm] (shu)
    {\textbf{6-Category SHU}\\[2pt]
     \hspace{6pt}Cadangan\hspace{16pt}5\%\\
     \hspace{6pt}Jasa Modal\hspace{10pt}40\%\\
     \hspace{6pt}Jasa Usaha\hspace{13pt}40\%\\
     \hspace{6pt}Pendidikan\hspace{15pt}5\%\\
     \hspace{6pt}Pengurus\hspace{20pt}5\%\\
     \hspace{6pt}Kesejahteraan\hspace{5pt}5\%};

% Internal dashed arrows (Bian-style)
\draw[darr] (vault.east)  -- (govern.west);
\draw[darr] (govern.east) -- (loans.west);

\begin{pgfonlayer}{background}
    \node[draw=blue!40, fill=blue!2, rounded corners=5pt,
          fit=(vault)(govern)(loans)(vrole)(grole)(lrole)(shu),
          inner sep=8pt, label={[font=\tiny\bfseries]above:Contract Layer}] {};
\end{pgfonlayer}

% === ARROW DOWN ===
\draw[arr] (0,-1.6) -- (0,-2.2);

% === BLOCKCHAIN LAYER ===
\node[box, fill=orange!12, minimum width=10.5cm]
    (chain) at (0,-2.6) {DChain --- EVM-Compatible --- Chain ID 17845 --- PoS Consensus};
\begin{pgfonlayer}{background}
    \node[draw=orange!50, fill=orange!3, rounded corners=5pt,
          fit=(chain), inner sep=6pt,
          label={[font=\tiny\bfseries]above:Blockchain Layer}] {};
\end{pgfonlayer}
...
Pasal 18(2) & LakomiToken.\texttt{transfersEnabled = false} & \ding{51} \\
Pasal 19--21 & LakomiToken.\texttt{resignMembership()} & \ding{51} \\
Pasal 22(1) & \texttt{getVotingPower()} $\rightarrow$ 1 & \ding{51} \\
Pasal 22(2), 41 & LakomiVault.\texttt{paySimpananPokok/Wajib()} & \ding{51} \\
Pasal 23 & LakomiGovern.\texttt{quorumNumerator = 67} & \ding{51} \\
...
Pasal 31 & LakomiToken.\texttt{revokeMembership()} via governance & \ding{51} \\
Pasal 33--35 & LakomiGovern.\texttt{executeDissolution()} & \ding{51} \\
Pasal 38--39(2) & \texttt{vetoProposal()} + PENGAWAS\_ROLE & \ding{51} \\
Pasal 41(1--3) & Simpanan Pokok, Wajib, Sukarela & \ding{51} \\
Pasal 43 & LakomiVault.\texttt{danaCadangan} tracking & \ding{51} \\
Pasal 45(1--2) & LakomiVault.\texttt{distributeSHU()} 6 categories & \ding{51} \\
Pasal 46--47 & LakomiVault.\texttt{generateFinancialSnapshot()} & \ding{51} \\
Permenkop 8/2023 Ps 6--7 & Tiered LTV + 5\% APY flat & \ding{51} \\
\bottomrule
\end{tabular}
\end{table*}
...
Key implementation decisions include: (1) the membership registry (\texttt{isRegisteredMember}) as a boolean mapping decoupled from token balance, ensuring voting power is strictly identity-based per Pasal 22(1); (2) the vault's six-category SHU split as configurable basis points settable via governance, enabling cooperative-specific adjustments over time; (3) the election mechanism using nested mappings for candidates, votes, and terms, with automatic role granting on finalization; and (4) the loans contract's tiered LTV calculation based on cumulative simpanan in the vault, with auto-approval for small loans (up to 200 USDC) to enable rapid emergency fund access.

Each contract was initialized with five Anvil accounts assigned to distinct roles: PENGAWAS\_ROLE (supervisor), APPROVER\_ROLE (loan approval), MEMBERSHIP\_ROLE (member management), TREASURER\_ROLE (vault operations), and GOVERN\_ROLE (SHU distribution), with the deployer holding DEFAULT\_ADMIN\_ROLE for initial configuration.

\section{Evaluation}

\subsection{Functional Compliance Verification}

All 26 UU 25/1992 provisions were verified through functional testing on a local DChain-equivalent Anvil chain~\cite{foundry}. Each provision was mapped to a specific test scenario that validates both the successful path and edge cases (e.g., duplicate registration rejection, non-member vote blocking, double-claim prevention). Table~\ref{tab:test-results} presents representative verification results.
...
18(2) & Token transfer attempted $\rightarrow$ reverted (\texttt{transfersEnabled == false}) & Pass \\
22(1) & Two members query voting power $\rightarrow$ both return 1 & Pass \\
22(2), 41 & Simpanan pokok paid $\rightarrow$ \texttt{simpananPokok[member] > 0} & Pass \\
23 & Proposal with 3 members, 2 votes $\rightarrow$ quorum met (2/3 $\geq$ 67\%) & Pass \\
...
31 & Governance proposal $\rightarrow$ vote $\rightarrow$ execute \texttt{revokeMembership()} & Pass \\
...
Table~\ref{tab:gas} presents gas costs for key cooperative operations, measured on a local DChain-equivalent chain. Deployment costs range from 1.58M to 2.22M gas per contract, with operational costs between 72K and 210K gas.
...
Deployment costs are one-time expenses that cooperatives bear during initial system setup. Operational costs---ranging from \$0.14 for simpanan payment to \$0.42 for loan requests---are sufficiently low for practical cooperative use, even at significant member scale.
...
\toprule
\textbf{Feature} & \textbf{Traditional Koperasi} & \textbf{Token-Based DAO} & \textbf{Lakomi} \\
\midrule
