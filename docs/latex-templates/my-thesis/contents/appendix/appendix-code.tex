...
\footnotesize
\begin{tabular}{p{0.18\textwidth}p{0.12\textwidth}p{0.14\textwidth}p{0.14\textwidth}p{0.28\textwidth}}
\toprule
\textbf{Nama Kontrak} & \textbf{Lines of Code} & \textbf{Inherits} & \textbf{Roles} & \textbf{Fungsi Utama} \\
\midrule
LakomiToken & \textasciitilde 230 & ERC20, AccessControl, ReentrancyGuard & DEFAULT\_ADMIN, MEMBERSHIP, MINTER, BURNER, LOCKER & registerMember, resignMember, revokeMembership, lockCollateral \\
\midrule
LakomiVault & \textasciitilde 410 & AccessControl, ReentrancyGuard, Pausable & DEFAULT\_ADMIN, TREASURER, GOVERN, PENGAWAS, LOAN & paySimpananPokok, paySimpananWajib, deposit, distributeSHU, claimSHU, getPengawasAuditReport \\
\midrule
LakomiGovern & \textasciitilde 280 & AccessControl, ReentrancyGuard, Pausable & DEFAULT\_ADMIN, PENGAWAS, GOVERN & createProposal, castVote, execute, vetoProposal, beginElection, finalizeElection, scheduleAnnualRAT \\
\midrule
LakomiLoans & \textasciitilde 270 & AccessControl, ReentrancyGuard, Pausable & DEFAULT\_ADMIN, APPROVER & requestLoan, approveLoan, disburse, repayInFull, markDefaulted, claimCollateral \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
	...
	\begin{enumerate}
		\item \textit{Deploy} MockUSDC sebagai \textit{stablecoin} untuk pengujian.
		\item \textit{Deploy} LakomiToken dengan parameter dasar.
		\item \textit{Deploy} LakomiVault dengan parameter: alamat USDC, jumlah simpanan pokok, periode simpanan wajib, jumlah simpanan wajib, \textit{threshold} penarikan, dan \textit{timelock} penarikan.
		...
		\item \textit{USDC approval}: menyetujui pengeluaran USDC oleh LakomiLoans dari LakomiVault.
	\end{enumerate}
...
\midrule
MockUSDC & $\sim$5 KB & $\sim$1.200.000 & Rendah \\
...
\midrule
Simpanan Pokok & 100 USDC & \texttt{Govern} & Dibayarkan sekali saat pendaftaran \\
Simpanan Wajib & 10 USDC/30 hari & \texttt{Govern} & Dibayarkan berkala oleh anggota \\
...
\textit{Auto-Approve} & $\leq$200 USDC & \texttt{Govern} & Ambang persetujuan otomatis \\
...
\section{Diagram Alur Keadaan (State Machine)}

Gambar~\ref{fig:state-machine} mengilustrasikan diagram alur keadaan (\textit{state machine}) untuk tiga entitas utama dalam sistem Lakomi: keanggotaan, proposal, dan pinjaman. Diagram ini memberikan pandangan tingkat tinggi tentang siklus hidup setiap entitas dan transisi keadaan yang dimungkinkan oleh sistem.

\subsection{State Machine Keanggotaan}

\begin{verbatim}
                 ┌──────────┐
                 │  None    │ (belum terdaftar)
                 └────┬─────┘
                      │ registerMember() + bayar simpanan pokok
                      ▼
                 ┌──────────┐
         ┌───────│  Active  │───────────────┐
         │       └────┬─────┘               │
         │            │                     │
         │ resignMember()             revokeMembership()
         │ (sukarela, tidak ada        (proposal tata kelola)
         │  pinjaman aktif)                 │
         ▼                                 ▼
  ┌───────────┐                    ┌───────────┐
  │ Resigned  │                    │ Revoked   │
...
\subsection{State Machine Proposal}
...
\subsection{State Machine Pinjaman}
