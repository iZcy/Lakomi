...
\begin{enumerate}
	\item \textbf{\textit{Membership Layer}} --- LakomiToken (LAK) berfungsi sebagai \textit{registry} keanggotaan berbasis ERC-20. Setiap anggota yang terdaftar memiliki tepat satu suara dalam tata kelola, terlepas dari jumlah token yang dimiliki atau kontribusi simpanan. Kontrak ini mengelola empat peran: MINTER, BURNER, LOCKER, dan MEMBERSHIP.
	...
	\end{itemize}
	\item \textbf{\textit{Blockchain Layer}} --- Seluruh kontrak di-deploy pada DChain (EVM-compatible, Chain ID 17845), jaringan blockchain konsorsium pendidikan tinggi Indonesia.
\end{enumerate}
...
\noindent dengan $P$ adalah pokok pinjaman dalam USDC dan $D$ adalah durasi pinjaman dalam hari. Suku bunga 5\% APY (\textit{Annual Percentage Yield}) ditetapkan sebagai suku bunga tetap (\textit{flat rate}) yang lebih rendah dibandingkan suku bunga pinjaman komersial pada umumnya, sesuai dengan prinsip koperasi yang mengutamakan kesejahteraan anggota. Formula ini memastikan bahwa perhitungan bunga bersifat proporsional terhadap durasi---semakin singkat periode pinjaman, semakin kecil bunga yang dibebankan---sehingga memberikan insentif bagi anggota untuk melunasi pinjaman tepat waktu.
