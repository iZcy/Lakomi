"""Test only the 4 failed databases: sage, cambridge, oxford, jstor."""
import asyncio
import json
import time
...
async def test_database(ctx, page, config, test_info):
    ...
    try:
        ...
        try:
            ...
            filename = db_name + "_test.pdf"
            filepath = DOWNLOAD_DIR / filename
            await download.save_as(filepath)
            if filepath.exists() and filepath.stat().st_size > 1000:
                size = filepath.stat().st_size // 1024
                console.print(f"  [bold green]Direct download OK: {filename} ({size}KB)[/bold green]")
                return "PASS"
        except Exception:
            pass
        if download_triggered:
            console.print(f"  [red]Direct download failed[/red]")
            return "NO_PDF"
